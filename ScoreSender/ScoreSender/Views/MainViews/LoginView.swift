    ...
    func logIn() {
        ...
        self.session.login(withPhoneNumber: finalPhone, resignRequired: { error in
            if let error = error {
                ...
                MyAlerts().showTextInputPrompt(placeholder: "654321", title: "Enter code sent to iPhone", message: "", keyboardType: .numberPad, callback: { userPressedOk, code in
                    if userPressedOk {
                        self.session.tryVerificationCode(verificationCode: code, completion: { error in
                            if let error = error {
                                self.error = error
                                self.isShowingAlert = true
                                self.buttonDisabled = false
