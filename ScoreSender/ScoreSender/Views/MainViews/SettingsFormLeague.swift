...
struct SettingsFormLeague: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            Text("Creator phone number: \(String(curLeague.players[curLeague.creatorUID]?.phoneNumber ?? ""))")
                .fontWeight(.heavy)
                .font(.system(size: 16))
            ...
            Button(action: { self.isPresentingChangeProfileModal.toggle() }) {
                ...
                DisplayNameAndPhoto(username: self.curPlayer.displayName, image: self.curPlayer.image, isPresented: self.$isPresentingChangeProfileModal, title: "Edit Profile", userFinished: { displayName, image, userFinished in
...
//            }
            ...
            if curLeague.creatorUID == session.session!.uid {
                ...
                Button(action: { self.isPresentingEditLeagueModal.toggle() }) {
                    ...
                    DisplayNameAndPhoto(username: self.curLeague.name, image: self.curLeague.leagueImage ?? Constants.defaultLeaguePhoto, isPresented: self.$isPresentingEditLeagueModal, title: "Edit League", userFinished: { newName, image, userFinished in
                ...
                Button(action: {
                    ...
                    MyAlerts().showTextInputPrompt(placeholder: "6505551234", title: "Enter the demo user's phone number", message: "This is so that the user can eventually claim their leagues", keyboardType: .default, callback: { userPressedEnter, phoneNumber in
                        if userPressedEnter {
                            ...
                            if self.curLeague.phoneNumberToUID[self.phoneNumber] != nil {
                                self.alertMessage = "\(self.curLeague.players[self.curLeague.phoneNumberToUID[self.phoneNumber]!]!.displayName) is already using that phone number"
                                return
                            }
                ...
                .sheet(isPresented: $isPresentingDemoPlayerModal, content: {
                    DisplayNameAndPhoto(username: self.realName, image: nil, isPresented: self.$isPresentingDemoPlayerModal, title: "Demo Player", userFinished: { displayName, image, userFinished in
                        self.session.addDemoPlayer(toLeague: self.curLeague, displayName: displayName, realName: self.realName, image: image, phoneNumber: self.phoneNumber, completion: { error in
                            userFinished(error)
                        })
                    })
                })
